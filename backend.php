<?php
	...
	if (! isset ($_POST['NA']) || ! isset ($_POST['ND'])|| ! isset ($_POST['xn'])|| ! isset ($_POST['xp'])|| ! isset ($_POST['VL'])|| ! isset ($_POST['VH'])|| ! isset ($_POST['m'])|| ! isset ($_POST['A'])|| ! isset ($_POST['r']) || !isset($_POST['delEc'])) {
		...
		*/
		...
		$T = $_POST['T'];//Kelvin
		$Vrlow = $_POST['VL'];//Volts
		$Vrhigh = $_POST['VH'];//negative Volts
		$meff = $_POST['m'];//effective/reduced mass
		$meff*=9.11e-31;//to get it into kg
		$area = $_POST['A'];//um^2, for use with series r when importing data from file
		$rseries = $_POST['r'];//Ohms, for use with series r when importing data from file
		$delEc = $_POST['delEc'];
		$experimentalFile='experiments/'.$_POST['experFile'];
		...
		if ($NA == null || !is_numeric($NA) || $ND == null || !is_numeric($ND) || $xn == null || !is_numeric($xn) || $xp == null || !is_numeric($xp) || $T == null || !is_numeric($T) || $meff == null || !is_numeric($meff) || $Vrlow == null || !is_numeric($Vrlow) || $Vrhigh == null || !is_numeric($Vrhigh) || $area == null || !is_numeric($area) || $rseries == null || !is_numeric($rseries)) {
			...
			$Efn=$etan*$k*$T/$q;//eV, actually calculating Efn-Ecn
			$Efp=$etap*$k*$T/$q;//eV, actually calculating Efp-Evp
			$Vbi=$Efn+$Efp+$Egp/$q;
		                
			/*Opening Experimental Data & Parsing
			 * --------------------------------------------------------------------------*/
			$file=fopen($experimentalFile,"r");//THIS ASSUMES a voltage sweep from low (-ve) voltage to higher voltages!!!
